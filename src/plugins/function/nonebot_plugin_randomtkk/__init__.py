...
guess_tkk = on_command(cmd="答案是", rule=Rule(inplaying_check))
surrender_tkk = on_command(cmd="找不到唐可可", aliases={"唐可可人呢", "呼叫鲤鱼姐"}, rule=Rule(starter_check))
 
@random_tkk.handle()
async def _(event1:GroupMessageEvent, matcher: Matcher, event: MessageEvent, args: Message = CommandArg()):
    if not is_level_S(event1):
        await random_tkk.finish()
...
@guess_tkk.handle()
async def _(event1:GroupMessageEvent, event: MessageEvent, state: T_State = Depends(get_user_guess)): 
    if not is_level_S(event1):
        await random_tkk.finish()
    uid = str(event.user_id)
    pos = state["guess"]
...
async def _(event1:GroupMessageEvent, matcher: Matcher, event: MessageEvent):
    if not is_level_S(event1):
        await random_tkk.finish()
    uid = str(event.user_id)
