...
def _getLove():
    ...
    # data = str(requests.get(url, timeout=3).text).encode('utf-8').decode('utf-8')
    # if data in LoveTalkList:
    #     return
    # with open(f'{FILE_PATH}\\love.txt', 'a+', encoding='utf-8') as f:
    #     f.write(data + '\n')
    #     f.close()
    data = "x"
...
# 老婆生成类
def is_rightful(word):  # 判断是不是阳间文字
    for ch in word:
...
def getScence(self: WifeObj) -> str:
    ...
    data = json.load(open(f'{FILE_PATH}\\love.json', 'r', encoding='utf8'))
    msg = random.choice(data)# 普通级，获取情话
    return msg.replace('|', '\n')
